...
def calculate_word2vec_accuracy(c_word2vec: word2vecext.Word2Vec, c_validation_words, validation_sentences_indices,
    ...
    word2label = {c_validation_words[i]: labels[i] for i in range(len(c_validation_words))}
    ...
    for i in range(len(validation_sentences_indices)):
        ...
        for j in range(start_index, end_index):
            ...
            if not (c_validation_words[j] in index2word_set):
                # Skip words which are not in the dictionary
                continue
            word_to_vec_list = c_word2vec.wv.most_similar(positive=[c_validation_words[j]], topn=1)
            similar_words = list(map(lambda it: it[0], word_to_vec_list))

            if word2label[similar_words[0]] == labels[j]:
